...
(cl:in-package :cl-kme) ;(in-package :km)
(defvar *dbg* t)
...
(defmethod ki ((s Symbol) &optional (pre ""))
 ;(intern (ki (symbol-name s) pre))
  (intern (ki (underscore (safe-trim (symbol-name s))) pre)) ;:km ;new-j26 not needed if no km pkg
 ;;if (prefixp "*" s) s
...
(defun kin (s)
  ...
  (let ((i (ki s)))  ;2bad can't make where cls&relation names not *ed
    (if (symbolp i) i 
      (intern i ;:km ;new j24, to deal w/using packaged version now ;not needed if no km pkg
              ))))
;
...
(defun show- (s)
  "w/o*"
  (showme (intern s ;:km  ;not needed if no km pkg
                  ))) ;new j-26
;
(defun show2 (s)
  "show ins then cls"
...
(defmethod safe_v ((sy symbol))  ;already in utr2.lisp
  (let* ((s (symbol-name sy))
    ...
    (if (and (numberp p) (> p 1)) (intern (safe_v s)) ; :km ; ;new j-26 ;not needed if no km pkg
      sy)))
