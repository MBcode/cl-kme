...
;  (when *dbg* (warn "do not send safe_v a cons"))  ;consider mk-subins here
;  (cons (safe_v (car c)) (safe_v (cdr c)))) ;redef4now:
(defmethod safe_v ((c cons))
  ;(when *dbg* (warn "do not send safe_v a cons"))
 ;(cons (safe_v (car c)) (safe_v (cdr c)))
 (let ((s (to-str c)))
  (when *dbg* (warn "safe_v cons of len:~a" (len s)))
  s))
                     ...
                     (let* ((sn (safe_v (car-lv pr))) ;use slotname as the classname if embed ins;sv-cls /later
                            ...
                            (val (if (consp sval) ;(listp sval)  ;even w/consp still safe_v warns /FIX
                                  ;(sv_al_nn (gentemp (to-str i)) sval)
                                   ...
                                   ;soon do a make-subins-if-id-p
                                   (sv sval) ;don't bounce cons, assert whole thing(safe_v sval)
                                   ))) ;can do this after listp of strlst ;no/getting warn;oh !lst
                       (when val
                         (sv i sn val)))))
    al))
...
(defun mk-i-al_s (i-al_s cls)   ;using this, but need more hints, maybe from cls description?
...
(defun thing-p (cls)  ;for now ;from tc.cl
  (let ((sc (gvs cls "superclasses")))
    (and (symbolp sc) (not (eq sc '|Thing|))))) 
...
;more from tc.cl
(defun interns (s) (if (stringp (intern s)) s))
(defun get-super (cls)
  (get-vals (interns cls) '#$superclasses :situation +global-situation+)) 

;defmethod ka ((s String))  ;like eval-str2km above
(defun kas (s)  ;like eval-str2km above
  "KM assert string"
 (when (full s) ;so no ""
  (let* ((sl (if (prefixp "(" s) s
          (strl s)))
     (ret (eval-str (str-cat "(km '#$" (rm_comma sl) ")"))))
   ;(if (full1 ret) (first-lv ret) ret)
      ret ;get the whole list now
    ))) ;if 1thing get it, else get list, ka+  
;or
;might try redef method,&hopefully not breaking anything
;#+IGNORE ;(gv- cls "superclasses") ;only giving direct super right now
;defmethod ka ((s String))  ;like eval-str2km above
